...
function downloadCasosCsv(rows: CasoPrioritario[]): void {
  const header = toCsvRow([
    "id_contrato", "departamento", "municipio", "entidad", "proveedor",
    "modalidad", "anio", "valor_contrato", "score", "nivel_riesgo", "urlproceso",
  ]);
  const body = rows.map((r) =>
    toCsvRow([
      r.id_contrato, r.dpto, r.municipio, r.nombre_entidad, r.nombre_proveedor,
      r.modalidad, r.anio, r.valor_contrato, r.score, TIER_LABELS[r.tier], r.urlproceso,
    ]),
  );
  ...
  a.href = url;
  a.download = "casos_prioritarios.csv";
  a.click();
  URL.revokeObjectURL(url);
}

export function CasosPrioritariosPage() {
  const { items, loading, loadedChunks, totalChunks, nItemsTotal, error } = useCasosPrioritarios();
  ...
  const columns = useMemo<ColumnDef<CasoPrioritario>[]>(
    () => [
      { accessorKey: "dpto", header: "Departamento", cell: (c) => c.getValue<string | null>() ?? "—" },
      { accessorKey: "municipio", header: "Municipio", cell: (c) => c.getValue<string | null>() ?? "—" },
      {
  ...
  if (loading && items.length === 0) return <LoadingState label="Cargando casos prioritarios…" />;
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">Casos prioritarios</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {nItemsTotal !== null ? `Top ${formatInt(nItemsTotal)} contratos por score de riesgo` : "Cargando…"}
            {loading && totalChunks && (
        ...
        <button
          onClick={() => downloadCasosCsv(filtered)}
